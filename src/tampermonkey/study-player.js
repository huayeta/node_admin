...
class MYPlayer {
    ...
    async init(srcs) {
        // 加载css
        cssLoader.load('./player/styles.css?t=1');
        //加载html
        const content = `
            <!-- Top Info -->
            <div id="title">
        ...
        // 加载js
        await import('./player/howler.core.js');
        await import('./player/siriwave.js');
 ...
 */
        ...
        srcs=srcs.map(src=>{
            return  {...src,howl:null}
        })
        this.srcs=srcs;
        var player = new Player(srcs);
        this.player = player;
